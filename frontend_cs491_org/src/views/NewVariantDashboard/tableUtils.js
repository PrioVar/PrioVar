...
export const filterRows = (data, config) => {
    return data.filter(row => {
        ...
        const scoreInRange = row.priovar_score >= config.scoreRange[0] && row.priovarScore <= config.scoreRange[1];
        //const gtMatch = config.gts.length === 0 || config.gts.includes(row.gt);
        //const acmgScoreMatch = config.strengths.length ? config.strengths.includes(row.acmgScore.split(', ')[1].split(': ')[1]) : true;
