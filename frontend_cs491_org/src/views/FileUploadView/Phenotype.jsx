...
import {
  ...
  const TABS = ['Dashboard', 'Gene Coverage', 'Sample Details', 'Analysis']
          ...
          {fileDetails ? (
            <Box flexDirection="column" spacing={3} mt={2}>
              ...
              <TabPanel value={tab} index={0}>
                {fileDetails?.vcf_id ? (
                  <Typography>Dashboard is not available for VCF Files.</Typography>
                ) : (
                  <Stack key={`dashboard-tabPanel-${0}`}>
                    <VariantOverview file={fileDetails} sampleName={sampleName} fileId={fileId} />
                  </Stack>
                )}
              </TabPanel>
              <TabPanel value={tab} index={1}>
                {fileDetails?.vcf_id ? (
                  <Typography>Gene Coverage is not available for VCF Files.</Typography>
                ) : (
                  <Stack key={`dashboard-tabPanel-${1}`}>
                    <GeneCov fileId={fileId} />
                  </Stack>
                )}
              </TabPanel>
